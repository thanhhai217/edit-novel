...
def edit_with_gemini(content: str, prompt_path: str = "prompt.txt") -> str:
    ...
    """
    print(f"[GEMINI] Bắt đầu biên tập nội dung, độ dài: {len(content)} ký tự")
    ...
    try:
        ...
        print("[GEMINI] Đang gửi yêu cầu đến Gemini API...")
        resp = requests.post(url, headers=headers, json=data, timeout=60)
        resp.raise_for_status()
        result = resp.json()
        # Lấy kết quả trả về
        edited_text = result["candidates"][0]["content"]["parts"][0]["text"]
        print(f"[GEMINI] Đã nhận kết quả, độ dài: {len(edited_text)} ký tự")
        return edited_text
